...
for i in range(num_boreholes):
    ...
    for j in range(num_layers):
        ...
        with col1:
            Sdepth = st.number_input(f"Depth of Soil Layer {j+1} (in m):", min_value=0.0, step=0.1, key=f"Sdepth_{i}_{j}")
        
        with col2:
            gamma = st.number_input(f"Bulk Density of Soil Layer {j+1} (in kg/m³):", min_value=0.0, step=0.1, key=f"gamma_{i}_{j}") * 9.81  # Convert to kN/m³
        
        with col3:
            cohesion = st.number_input(f"Cohesion of Soil Layer {j+1} (in kPa):", min_value=0.0, step=0.1, key=f"cohesion_{i}_{j}") * 98.1
        
        with col4:
            friction_angle = st.number_input(f"Angle of Friction for Soil Layer {j+1} (in degrees):", min_value=0, max_value=50, step=1, key=f"friction_angle_{i}_{j}")
    ...
    elif Lpile <= layers[0]['Depth (m)'] + (layers[1]['Depth (m)'] if len(layers) > 1 else 0):
        d2 = Lpile - layers[0]['Depth (m)']
        PD = (layers[0]['γ (kN/m³)'] * layers[0]['Depth (m)']) + (layers[1]['γ (kN/m³)'] * d2 if len(layers) > 1 else 0)
        Qub_g = Ap * (0.5 * Dpile * layers[1]['γ (kN/m³)'] * layers[1]['Nγ'] + PD * layers[1]['Nq'] if len(layers) > 1 else 0)
        Qub_c = Ap * layers[1]['C (kPa)'] * layers[1]['Nc'] if len(layers) > 1 else 0
    else:
        d3 = Lpile - (layers[0]['Depth (m)'] + (layers[1]['Depth (m)'] if len(layers) > 1 else 0))
        PD = (layers[0]['γ (kN/m³)'] * layers[0]['Depth (m)']) + (layers[1]['γ (kN/m³)'] * layers[1]['Depth (m)'] if len(layers) > 1 else 0) + (layers[2]['γ (kN/m³)'] * d3 if len(layers) > 2 else 0)
        Qub_g = Ap * (0.5 * Dpile * layers[2]['γ (kN/m³)'] * layers[2]['Nγ'] + PD * layers[2]['Nq'] if len(layers) > 2 else 0)
        Qub_c = Ap * layers[2]['C (kPa)'] * layers[2]['Nc'] if len(layers) > 2 else 0
